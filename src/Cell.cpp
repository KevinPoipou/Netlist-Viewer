...
namespace Netlist {
    ...
    Cell::Cell(const string & name): symbol_(this) // TME7
    ...
    Cell * Cell::fromXml(xmlTextReaderPtr reader) {
        enum State {
            Init = 0, BeginCell, BeginTerms, EndTerms, BeginInstances, EndInstances, BeginNets, EndNets, BeginSymbol, EndCell, ParseError
        };
        ...
        while (true) {
            ...
            case Init:
                if (cellTag == nodeName) {
                    state = BeginCell;
                    string cellName = xmlCharToString(xmlTextReaderGetAttribute(reader, (const xmlChar * )
                        "name"));
