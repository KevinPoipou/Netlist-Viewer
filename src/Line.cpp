...
namespace Netlist {
    ...
    Line::Line(Node * source, Node * target): source_(source), target_(target) {
        source_ -> attach(this);
        target_ -> attach(this);
    }
    ...
    bool Line::fromXml(Net * net, xmlTextReaderPtr reader) {
        ...
        if (lineTag == nodeName) {
            ...
            if (not source) {
                cerr << "[ERROR] Line::fromXml(): Unknown source node id:" << idSource << " (line:" << xmlTextReaderGetParserLineNumber(reader) << ")." << endl;
                return false;
            }
            if (not target) {
                cerr << "[ERROR] Line::fromXml(): Unknown target node id:" << idTarget << " (line:" << xmlTextReaderGetParserLineNumber(reader) << ")." << endl;
                return false;
            }
