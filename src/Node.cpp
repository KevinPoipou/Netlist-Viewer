...
namespace Netlist {
    ...
    NodeTerm::NodeTerm(Term * term, size_t id): Node(id), term_(term) {}
	...
	void  NodeTerm::toXml ( ostream& stream ) const{
		...
		//if (term_->getName().empty()) stream << "\" x=\"" << position_.getX() << "\" y=\"" << position_.getY() << "\"/>\n";
		//stream << "\"/>\n";
    ...
    bool Node::fromXml(Net * net, xmlTextReaderPtr reader) {
        ...
        if (nodeTag == nodeName) { 
            ...
            if (not instanceName.empty()) {
                ...
                if (not instance) {
                    cerr << "[ERROR] No Instance <" << instanceName << "> in Cell <" << net -> getCell() -> getName() << "> (line:" << xmlTextReaderGetParserLineNumber(reader) << ")." << endl;
                    return false;
                }
                term = instance -> getTerm(termName);
                if (not term) {
                    cerr << "[ERROR] No Term <" << termName << "> in Instance <" << instanceName << "> (line:" << xmlTextReaderGetParserLineNumber(reader) << ")." << endl;
                    return false;
                }
            } else {
                term = net -> getCell() -> getTerm(termName);
                if (not term) {
                    cerr << "[ERROR] No Term <" << termName << "> in Cell <" << net -> getCell() -> getName() << "> (line:" << xmlTextReaderGetParserLineNumber(reader) << ")." << endl;
                    return false;
                }
            }
